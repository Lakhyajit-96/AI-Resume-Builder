...
import { Briefcase, Plus, Trash2, Sparkles, LoaderCircleIcon } from 'lucide-react';
...
const ExperienceForm = ({ data = [], onChange = () => {} }) => {
    ...
    const enhanceDescription = async (index) => {
        ...
        try {
            ...
            const base = `${exp.position || ''} at ${exp.company || ''}`.trim();
            const seed = exp.description && exp.description.trim() ? exp.description : `Write a concise, ATS-friendly 1-2 sentence job description for the role ${base}. Focus on impact and quantifiable achievements.`;
            const token = localStorage.getItem('token');
            const { data: res } = await api.post('/api/ai/enhance-job-desc', { userContent: seed }, { headers: { Authorization: token } });
                        ...
                        <div key={index} className="p-4 border border-gray-200 rounded-lg space-y-3">
                            ...
                            <div className="space-y-2">
                                <div className="flex items-center justify-between">
                                    ...
                                    <button type="button" onClick={() => enhanceDescription(index)} disabled={enhancingIndex !== null} className="flex items-center gap-1 px-2 py-1 text-xs bg-purple-100 text-purple-700 rounded hover:bg-purple-200 transition-colors disabled:opacity-50">
